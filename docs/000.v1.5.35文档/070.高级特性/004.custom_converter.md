...
permalink: /pages/1.5.35/custom_converter/
---
...
同上文的 《[springboot 下配置全局转换器](/pages/1.5.35/converter/#springboot-下配置全局转换器)》一样，可通过`@Bean`注解方法将自定义转换器实例注入到 Spring 上下文
...
也同上文的 《[配置接口/方法级别转换器](/pages/1.5.35/converter/#配置接口-方法级别转换器)》一样，通过`@BodyType`注解来指定自定义的转换器类
