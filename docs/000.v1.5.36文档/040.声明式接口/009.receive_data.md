...
permalink: /pages/1.5.36/receive_data/
---
...
在使用自定义类型作为接口返回类型，接受服务端响应的数据时，往往会碰到接收到的字段名与 Java 类中字段名的命名风格不匹配的情况（比如接收到的是 user_id, 而 Java 类中却是 userId），对于此类问题请参见《[对象字段](/pages/1.5.36/object_field/)》
...
:::tip 文档导航
要详细了解如何手动执行请求发送，请参见《[执行请求](/pages/1.5.36/api_execute/)》
:::
...
:::tip 文档导航
拦截器详细内容请参见《[拦截器](docs/1.5.36/interceptor)》
:::
