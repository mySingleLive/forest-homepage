...
permalink: /pages/1.5.30/http_body/
---
...
在使用 Protobuf 之前，首先需要有引入 google 的 protobuf 包依赖， 如何加相关依赖请参见《[添加Protobuf框架依赖](/pages/1.5.30/spring_boot_install/#protobuf框架依赖)》
...
:::tip 文档导航
关于更多二进制传输以及文件上传内容，请见《[上传下载](/pages/1.5.30/upload_download/)》
:::
...
:::tip 提示
其中${数字}的语法用到了《[参数序号引用](/pages/1.5.30/refer_index/)》
:::
...
在 Lambda 参数中可以调用`req.body().encode()`可以立即序列化整个请求体的数据
