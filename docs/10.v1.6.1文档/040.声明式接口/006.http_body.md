...
permalink: /pages/1.6.1/http_body/
---
...
在使用 Protobuf 之前，首先需要有引入 google 的 protobuf 包依赖， 如何加相关依赖请参见《[添加Protobuf框架依赖](/pages/1.6.1/spring_boot_install/#protobuf框架依赖)》
...
:::tip 文档导航
关于更多二进制传输以及文件上传内容，请见《[上传下载](/pages/1.6.1/upload_download/)》
:::
...
在使用自定义类型作为 Body 参数时，往往会碰到要发送的字段名、顺序和 Java 类中的字段名、顺序不同的情况，对于此类问题请参见《[对象字段](/pages/1.6.1/object_field/)》