...
可以直接在自定义 SSE 控制器中添加 SSE 的事件消息处理方法（当然也可以在 [SSE 拦截器](/pages/1.6.x/sse_interceptor/)中添加），然后通过`@SSEDataMessage`注解、`@SSEEventMessage`注解、`@SSEIdMessage`注解、`@SSERetryMessage`注解来修饰自定义的方法，而这些方法根据所标注的注解可以接受和处理不同类型名称的事件消息。
