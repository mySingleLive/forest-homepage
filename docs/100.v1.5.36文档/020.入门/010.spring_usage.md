...
permalink: /pages/1.5.36/spring_usage/
---

在 Forest 依赖加入好之后，就可以构建 HTTP 请求的接口了

如还没有添加好依赖或配置好，请参见《[Spring环境安装](/pages/1.5.36/spring_install/)》和《[Spring环境配置](/pages/1.5.36/spring_config/)》
...
在上一节内容中已经进行了对接口所在包扫描的配置，如没有请参见《[Spring环境配置](/pages/1.5.36/spring_config/)》
