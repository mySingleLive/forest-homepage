...
在使用 Protobuf 之前，首先需要有引入 google 的 protobuf 包依赖， 如何加相关依赖请参见《[添加Protobuf框架依赖](/pages/1.5.33/spring_boot_install/#protobuf框架依赖)》
...
/**
 * 发送Byte数组类型数据
 * 注：当body为Java类，需要将类转成json byte数组传入body
 */
@Post(
        url = "/upload/${filename}",
...
:::tip 文档导航
关于更多二进制传输以及文件上传内容，请见《[上传下载](/pages/1.5.33/upload_download/)》
:::
