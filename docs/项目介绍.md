...
声明式接口能够将 HTTP 的所有请求信息（包括 URL、Header 以及 Body 等信息）绑定到您自定义的 Interface 方法上，能够通过调用本地接口方法的方式发送 HTTP 请求。
...
// 声明注册用户的 POST JSON 数据请求的接口
@Post(url ="/user/register", contentType = "application/json")
String registerUser(@Body("username") String username, @Body("password") String password);
```

#### 编程式

Forest 提供了非常灵活方便的编程式 API 接口，能让开发者通过链式调用的方式发送 HTTP 请求。
...
5. 数据转换： 此模块将字符串数据和`JSON`或`XML`形式数据进行互转。目前 JSON 转换器支持`Jackson`、`Fastjson2`、`Fastjson`、`Gson`四种，XML 支持`JAXB`一种。
